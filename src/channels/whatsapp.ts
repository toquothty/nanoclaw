      ...
      sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
          ...
          if (shouldReconnect) {
            ...
            // Connection closed without a genuine logout — almost always a
            // clean shutdown (SIGTERM). Keep the keystore intact so the linked
            // device survives the restart; only a real loggedOut (401) above
            // should wipe auth. (Previously this branch deleted authDir on
            // every shutdown, forcing a re-pair after each restart.)
            log.info('WhatsApp connection closed without logout — keeping auth', { shuttingDown });
            if (rejectFirstOpen) {
              rejectFirstOpen(new Error('WhatsApp connection closed'));
              rejectFirstOpen = undefined;
              resolveFirstOpen = undefined;
            }
          }
